...
class DatabaseService {
  ...
  private async request(endpoint: string, method: string = "GET", body?: any) {
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        throw new Error(
          `Server returned ${res.status} ${res.statusText}. Response: ${text.substring(0, 100)}...`,
        );
      }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      // Request: /api/health -> Proxy -> https://rapido-ride-six.vercel.app/api/health
      await this.request("/health");
      return true;
    } catch (e) {
      console.error(
        "Health Check Failed. Ensure the backend 'server.js' is running on port 4000.",
      );
      return false;
    }
  }
  ...
  async updateProfile(fullName: string, profileImage: string): Promise<User> {
    return await this.request("/users/profile", "PUT", {
      fullName,
      profileImage,
    });
  }
